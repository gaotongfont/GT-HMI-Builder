...
void bsp_uart3_init(void)
{
    ...
    PORT_Init(UART3_RX_PORT, UART3_RX_PIN, UART3_RX_SEL, 1);	//GPIOB.4配置为UART3输入引脚
    PORT_Init(UART3_TX_PORT, UART3_TX_PIN, UART3_TX_SEL, 0);	//GPIOB.3配置为UART3输出引脚
...
void UART3_Handler(void)
{
    ...
    if(UART_INTStat(UART3, UART_IT_RX_TOUT))//超时中断，相当于空闲中断
    {
		while(UART_IsRXFIFOEmpty(UART3) == 0)//判断FIFO缓存区是否为空
        {
            if(UART_ReadByte(UART3, &chr) == 0)//接收数据
            {
                if(_uart3_count < _UART3_RX_BUFFER_SIZE)
