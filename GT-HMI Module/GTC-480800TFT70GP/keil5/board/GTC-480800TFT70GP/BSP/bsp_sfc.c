...
//void spi_wr(uint8_t * dw, uint32_t lw, uint8_t * dr, uint32_t lr)
//{
//		
//}
...
{
	// unsigned char mod = 0 , *q;;
	// unsigned long count = 0;
	// uint32_t *p = (uint32_t*)pBuff , tmp = 0;

	// count = DataLen / 4 ;
	// mod = DataLen % 4 ;
	// SFC_Read(address, p , count);
	// if(mod)
	// {
	// 	q = (unsigned char*)(p + count) ;
	// 	SFC_Read(address + (count * 4), &tmp , 1);
	// 	while (mod--)
	// 	{
	// 		*q = (unsigned char)(tmp & 0xFF) ;
	// 		tmp >>= 8 ;
	// 		q++;
	// 	}
	// }
	...
	memcpy(pBuff,(volatile unsigned int *)(0x80368000+cnt),DataLen);
