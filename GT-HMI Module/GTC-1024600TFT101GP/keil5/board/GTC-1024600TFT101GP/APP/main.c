...
void _flush_v_cb(struct _gt_disp_drv_s * drv, gt_area_st * area, gt_color_t * color) {
	uint32_t x = 0, y, j = 0;
	for (y = 0; y < area->h; y++) {
		for (x = 0; x < area->w; x++) {
			_LCD_Buffer[(LCD_VDOT - (x + area->x) - 1) * LCD_HDOT + (y + area->y)] = color->full;
			color++;
		}
	}
}
void read_cb(struct _gt_indev_drv_s * indev_drv, gt_indev_data_st * data) {
	if (!GT911_Scan(0)) {
		...
		// GT_LOGD(GT_LOG_TAG_TEST, "read_cb: GT_INDEV_STATE_RELEASED");
		return;
	}
