...
{
	screen_xg = gt_obj_create(NULL);
	gt_obj_add_event_cb(screen_xg, screen_setup_0_cb, GT_EVENT_TYPE_INPUT_HOME_GESTURE_LEFT, NULL);
	gt_screen_set_bgcolor(screen_xg, gt_color_hex(0x000000));


	

	/** listvxg */
	listvxg = gt_listview_create(screen_xg);
	gt_obj_set_pos(listvxg, 19, 57);
	gt_obj_set_size(listvxg, 200, 166);
	...
	gt_listview_set_item_radius(listvxg, 6);
	...
	gt_listview_add_item(listvxg, "外向");
	gt_listview_add_item(listvxg, "正经");
	gt_listview_add_item(listvxg, "搞怪");
	gt_listview_add_item(listvxg, "内向");
